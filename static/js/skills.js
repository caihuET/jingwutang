...
function renderSlotGrid(slotted, allSkills) {
    ...
    for (var i = 1; i <= 4; i++) {
        ...
        if (s) { html += '<div class="value" style="font-size:16px">技能#' + s.skill_id + '</div><div class="label">Lv.' + s.level + '</div>'; }
...
function renderLearned(skills) {
    ...
    for (var i = 0; i < skills.length; i++) {
        var s = skills[i];
        html += '<tr><td>技能#' + s.skill_id + '</td><td>Lv.' + s.level + '</td><td>' + s.proficiency + '</td>';
